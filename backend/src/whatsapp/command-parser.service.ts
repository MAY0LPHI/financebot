...
export class CommandParserService {
  ...
  parseCommand(text: string): ParsedCommand {
    ...
    if (
      this.matchesPattern(lowerText, ['meta', 'metas', 'objetivo', 'objetivos', 'goal', 'goals'])
    ) {
  ...
  private extractTransactionDetails(text: string): {
    amount?: number;
    description?: string;
    categoryName?: string;
  } {
    let amount: number | undefined;
    const amountPatterns = [/R?\$?\s*([\d.,]+)/i, /([\d.,]+)\s*(?:reais|real|r\$)/i, /([\d.,]+)/];
    ...
    let description = text
      ...
      .replace(
        /(?:recebi|ganhei|gastei|paguei|comprei|entrada|saída|saida|despesa|receita|com|de|em|para|no|na)/gi,
        '',
      )
      .trim();
  ...
  private async addTransaction(
    ...
    if (!command.amount || command.amount <= 0) {
      return {
        success: false,
        message:
          'Não consegui identificar o valor. Por favor, inclua o valor na mensagem. Exemplo: "Gastei 50 com alimentação"',
      };
    }
    ...
    if (!account) {
      return {
        success: false,
        message:
          'Você não tem nenhuma conta cadastrada. Acesse o sistema web para criar uma conta primeiro.',
      };
    }
    ...
    const transaction = await this.prisma.transaction.create({
      data: {
        ...
        description:
          command.description || `${type === 'INCOME' ? 'Receita' : 'Despesa'} via WhatsApp`,
        date: new Date(),
      },
  ...
  private async getGoals(userId: string): Promise<CommandResult> {
    ...
    if (goals.length === 0) {
      return {
        success: true,
        message:
          '🎯 Você não tem nenhuma meta cadastrada. Acesse o sistema web para criar suas metas!',
      };
    }
